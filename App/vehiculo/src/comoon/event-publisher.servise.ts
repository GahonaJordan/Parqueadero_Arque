...
export class EventPublisher implements OnModuleInit, OnModuleDestroy {
    ...
    private async connect() {
        ...
        const hostsToTry = host === 'localhost' ? [host, 'host.docker.internal'] : [host];
    ...
    async publishEvent(event: AuditEvent) {
        if (!this.channel) {
        this.logger.warn('Channel is not established. Attempting to connect...');
        await this.connect();

        if (!this.channel) {
            this.logger.error('RabbitMQ channel is still unavailable. Event was not published.');
            return;
        }
        }
        ...
        try {
            ...
            this.logger.debug(`Eveneto publicado: ${event.servicio} - ${event.accion} - ${event.entidad}`);
        }catch (error){
